...
**输出**：结构化数据行，每行绑一个产品代码，附指标数值、来源、有效时点和取得时间——不是一篇散文。写入审查记录 Research 节；可选写入 `database/sources.csv`、`raw_material/`、`reports/`。
...
**输入**：Validation 放行后的数据（带代码 + 状态标签）；模型版本；IPS 中的阈值约定。
