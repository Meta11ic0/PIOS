...
`products/schema.yaml` 是统一产品字段基线，当前 `schema_version: 0.4.0`（0.3.0→0.4.0 变更：补齐跨境 ETF 币种/额度与动态指标口径，区分基础字段与动态字段）。各资产类型可增加特有字段，但不要修改通用字段含义。发现错误时追加更正记录，不覆盖原始动态快照。
