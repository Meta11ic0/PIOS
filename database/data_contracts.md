...
- 字段 `as_of` 表示事实适用时点；字段 `published_at` 表示来源发布时间；字段 `retrieved_at` 表示取得时间。时间使用带时区的 RFC 3339。
...
| 汇率，持仓折算 | 与估值适用时点同一可比日 | 须记录 `fx_quote_convention`；估值适用时点对应字段 `valuation_as_of` |

## 数据集契约

| 数据集 | 主键或去重键 | 必填血缘 | 说明 |
|---|---|---|---|
| `sources.csv` | `source_id` | URL 或受控文件引用、来源等级、取得时间、`scope` | 来源登记，不等同于事实结论；可选 `raw_material_path` 指向待蒸馏材料。取得时间对应字段 `retrieved_at`。 |
| `portfolio/holdings.csv` | `record_id` | `source_id`、估值和 FX 来源/时点 | 追加式持仓快照；同一 `holding_id` 可有多个适用时点。交易、基金、底层暴露和报告币种不得混用。 |
...
| `products/history/etf_metrics.csv` | `record_id` | `source_id`、适用时点、取得时间、指标口径 | 动态指标快照；市价、NAV/IOPV 时间点、折溢价/价差/跟踪窗口分别记录。适用时点与取得时间对应字段 `as_of`、`retrieved_at`。 |
| `products/history/asset_profile_exposure.csv` | `record_id` | `source_id`、适用时点、取得时间、权重口径与覆盖 | 产品穿透暴露快照；每条记录只表达一个维度值。适用时点与取得时间对应字段 `as_of`、`retrieved_at`。 |
...
2. **时间**：动态事实有适用时点；取得时间不早于来源发布时间，有值时校验；旧决策不使用决定日之后的事实解释；关键字段未超最大允许时效。适用时点、取得时间、来源发布时间对应字段 `as_of`、`retrieved_at`、`published_at`。
