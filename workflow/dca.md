...
状态：active  
适用范围：已纳入目标配置的场内 ETF 定期定额（或定期不定额）买入
...
3. 单笔金额上限与频率上限；
4. 失效条件与复核触发器；
5. 绑定的 active IPS 与有效 `allocation_set_id`。
...
2. Validation：关键动态数据（折溢价、价差、流动性、申赎/交易状态、QDII 等）在字段最大允许时效内；任一关键项过期、冲突或 `unknown` 即回退完整路径（不得用 `warning` 放行后执行）。
...
6. Challenge：仍遵循 [skills/challenge/SKILL.md](../skills/challenge/SKILL.md) 全文（3 反例 + 3 替代 + 3 可能错误，`pass / revise / reject`）。
7. Decision：确认原结论仍在有效期内且本次金额未超授权边界；否则回退完整路径。
8. Documentation：优先追加到原 Decision Log；条件变化则新建。
9. 用户执行：仅当本轮授权仍有效（产品、方向、数量/限额、订单类型、有效期）时向用户呈现检查清单；Agent 不得下单。用户成交后更新持仓。

## 完整路径

新标的或超计划加仓时，改走 [buy_etf.md](buy_etf.md) 的完整流程（含委员会触发），或等价完成八步全量审查后再定投。
