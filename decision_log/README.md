...
生产 Decision 写在本目录；**演示**只放在 [`demo/`](demo/)，且必须 `scope: demo_only`。查询生产 Decision、复盘样本或授权先例时排除 `demo/`。

## 生命周期

1. **决定时**：冻结目标与约束、active `ips_id`、目标配置集、有效例外、上游 `review_id`/`run_id`、来源与事实快照、候选方案、风险、Challenge、决策边界、失效条件（含 `valid_until`）和复核时间；记录 `frozen_at` 与内容哈希。
...
- 触发器分为 `invalidation`、`action`、`review`。触发器只允许重新进入审查流程，绝不自动下单。

Decision Log 可记录 `act`、`wait`、`reject` 或 `research`；没有成交同样应保留记录，以便复盘过程质量。`unknown` 停止演示不是“审查通过”。
