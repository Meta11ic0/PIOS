...
委员会是 Review Pipeline 第 3–6 阶段的编排辅助，不是第九阶段，也不替代 Modeling、Reasoning、Risk 或 Challenge。它不产生交易授权，不创建常驻 Agent，不以投票或平均分代替事实核验。
...
1. IPS 必须为 `active` 且具有批准记录；持仓、有效目标配置、候选动作和适用例外可定位。任何缺失、草案或过期例外均标记 `unknown`。
2. 产品身份、来源、口径和动态数据已经过 Validation；关键 `fail/unknown` 直接停止。
3. 输入包冻结：记录 `ips_id`、持仓/目标/偏离快照、候选动作、产品与指数资料、`source_id`、`as_of`、硬约束和模型版本。
...
- 输出：身份与数据逐项 `pass/warning/fail/unknown`，以及来源、`as_of`、口径、替代产品。
...
4. 剩余判断分歧必须按 `material` 或 `non_material` 分类；`material` 分歧默认 `wait/revise`，除非用户作为最终裁决者在 Decision Log 中明确接受剩余风险、裁决依据与复核条件。

## 统一输出

在 `templates/review.md` 的“决策委员会”节记录：
...
`C` 不允许进入 `act`。委员会汇总只提出配置路径、被排除替代、阻断项和复核条件；最终 Decision 仍按 `skills/decision/SKILL.md` 形成。
