...
A. 目标与战略配置席：判断方案是否服务期限、用途、现金流、风险预算和目标配置，当前是否真的需要行动。输出配置必要性、目标区间、偏离、再平衡规则和不行动理由。禁止用短期市场观点改写 IPS。

B. 资产暴露与组合结构席：判断新增的资产类别、地区、行业、风格、币种和重叠暴露是否符合组合结构。输出暴露变化、集中/重叠风险、替代资产或指数路径。禁止因近期表现强势绕过风险预算。

C. 产品实施与数据验证席：判断候选产品能否以可接受成本、流动性、跟踪、申赎状态、账户可达性和法规税务约束实现既定暴露；跨境产品还须核对准入限制、申赎状态、交易/计价/底层暴露币种、底层市场时差与估值滞后。输出身份与数据逐项 `pass`/`warning`/`fail`/`unknown`，以及来源、适用时点、口径、替代产品。禁止从「产品质量较高」推出「当前应配置该资产」。

D. 风险与反方席：判断不行动、现金、替代配置或更简单方案是否更优；哪些假设、数据或执行错误会推翻结论。输出至少三项反例、三种替代方案、三类可能错误，以及风险、触发器、影响、缓释、剩余风险和 `pass/revise/reject` 裁决。禁止以无关联的泛化风险否决全部行动。
